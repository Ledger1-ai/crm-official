...
const UpdateTaskDialog = ({ users, boards, boardId, initialData, onDone }: Props) => {
  ...
  useEffect(() => {
    ...
    async function loadOpps() {
      if (!board) return;
      try {
        const res = await fetch(`/api/campaigns/${encodeURIComponent(board)}/opportunities`, { cache: "no-store" });
        if (res.ok) {
  ...
  const onSubmit = async (data: UpdatedTaskForm) => {
    ...
    try {
      ...
      await axios.put(`/api/campaigns/tasks/update-task/${initialData.id}`, payload);

      // If an opportunity was chosen, link it to this task
      if (data.opportunityId && boardId) {
        await fetch(`/api/campaigns/${encodeURIComponent(boardId)}/opportunities/${encodeURIComponent(data.opportunityId)}/link-task`, {
          method: "POST",
