...
const Kanban = (props: any) => {
  ...
  const onDragEnd = async (result: DropResult) => {
    ...
    if (type === "COLUMN") {
      ...
      try {
        await axios.put(`/api/campaigns/sections/reorder-sections`, {
          list: newSections,
        });
        toast({
          title: "Section moved",
    ...
    try {
      ...
      await axios.put(`/api/campaigns/tasks/update-kanban-position`, {
  ...
  const onDeleteSection = async () => {
    ...
    try {
      await axios.delete(`/api/campaigns/sections/delete-section/${sectionId}`);
  ...
  const updateSectionTitle = async (
    ...
    timer = setTimeout(async () => {
      try {
        await axios.put(`/api/campaigns/sections/update-title/${sectionId}`, {
          newTitle,
        });
        toast({
  ...
  const createTask = async (sectionId: string) => {
    try {
      const { data: task } = await axios.post(
        `/api/campaigns/tasks/create-task/${boardId}`,
        {
          section: sectionId,
        }
      );
