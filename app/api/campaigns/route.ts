...
export async function GET() {
  console.log('[GET /api/campaigns] Request started');
  const session = await getServerSession(authOptions);
  if (!session) {
    console.log('[GET /api/campaigns] No session');
    return new NextResponse("Unauthenticated", { status: 401 });
  }
  try {
    const teamInfo = await getCurrentUserTeamId();
    console.log('[GET /api/campaigns] Team Info:', teamInfo);
    const teamId = teamInfo?.teamId;

    if (!session?.user?.id) {
      console.log('[GET /api/campaigns] Missing session.user.id');
    ...
    console.log('[GET /api/campaigns] orConditions:', JSON.stringify(orConditions));
